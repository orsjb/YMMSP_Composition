...
public class LiveCoder implements HBAction {
    ...
    public void action(HB hb) {
        ...
        Glide noiseLevel = new Glide(hb.ac, 0);
        WavePlayer modulator = new WavePlayer(hb.ac, 50, Buffer.SINE);
        ...
        hb.pattern(new Bead() {
            ...
            protected void messageReceived(Bead bead) {
                if (hb.clock.getCount() % 2 == 0) {
                    ...
                    freq = (float)(Math.pow(2, oct-1) * freq);
                    if (hb.rng.nextFloat() < 0.2f) freq *= 0.5f;
                    final float ffreq = freq;
                    Function mod = new Function(modulator, noiseLevel) {
                        @Override
                        public float calculate() {
                            return ffreq + x[0] * x[1] * 200f;
                        }
                    };
                    WavePlayer wp = new WavePlayer(hb.ac, mod, Buffer.SINE);
                    Envelope e = new Envelope(hb.ac, 0);
                    Gain g = new Gain(hb.ac, 1, e);
                    e.addSegment(hb.rng.nextFloat() * 0.1f + 0.2f, 50 * hb.rng.nextFloat());
...
//                    hb.sound(g);
                ...
                if (hb.clock.getCount() % 6 == 5) {
                    ...
                    Envelope e = new Envelope(hb.ac, 0);
                    Gain g = new Gain(hb.ac, 1, e);
                    e.addSegment(hb.rng.nextFloat() * 0.05f + 0.1f, 10);
...
//                }
                ...
                new GyroscopeListener(hb) {
                    ...
                    public void sensorUpdated(float pitch, float roll, float yaw) {
                        noiseLevel.setValue(Math.abs(pitch));
                        oct = (int)(Math.abs(pitch) * 4) % 4;
                    }
                };
